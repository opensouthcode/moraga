...
class ConferencesController < ApplicationController
  ...
  load_and_authorize_resource find_by: :short_title, except: [:show, :show_new]
  ...
  def show
    load_conference_splash_data
    redirect_to(admin_conference_splashpage_path(@conference.short_title)) && return unless @conference.splashpage.present?
    @image_url = "#{request.protocol}#{request.host}#{@conference.picture}"
    render :show_new if @conference.splashpage.use_new_design?
  end
  ...
  def load_conference_splash_data
    ...
    return unless splashpage.present?
    ...
    if splashpage.include_program
      ...
      if splashpage.include_tracks
        @tracks = @conference.confirmed_tracks.eager_load(:room).order('tracks.name')
      end
